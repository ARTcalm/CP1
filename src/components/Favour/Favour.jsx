...
export const Favours = (props) => {
    ...
    const {items, setItems, favoursItems, setFavoursItems} = props

    const handleDeleteFavour = (id) => {
        setItems(items => {
            return items.map(el => {
                if(el.id === id){
                    return {
    ...
    useEffect(()=>{
        items.map((el,index) => {
            if(el.favours){
                const isInArray = favoursItems.some(el => el.id === index)
                if(!isInArray){
                setFavoursItems(prev => [...prev, el])}
            }
            else{
                setFavoursItems(favoursItems => favoursItems.filter(el=> el.id !== index))
            }
        })
    },[items])
        ...
        <div className={style.favoursBG}>
                ...
                <div className={style.favoursContainer}>
                    <div className={style.favoursTrack}> 
                        {favoursItems.length === 0 ? <p>Избранные отсутствуют</p> :
                            favoursItems.map(el => (
