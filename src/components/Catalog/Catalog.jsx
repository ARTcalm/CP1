...
export const Catalog = (props) => {
    const {items, setItems,setFavoursItems, favoursItems} = props
    ...
    const handleAddCart = (item) => {// К ФУНКЦИИ ДОБАВЛЕНИЯ В КОРЗИНУ, ДОБАВИЛ УСЛОВИЕ ЧТОБЫ ТОВАРЫ НЕ ПОВТОРЯЛИСЬ В КОРЗИНЕ БОЛЬШЕ 1 РАЗА
        const isInArray = favoursItems.some(el => el.id === item.id)
        if(!isInArray){
        setFavoursItems(prev => [...prev, item])}
    }

    const DeleteCartItems = (id) =>{
          setFavoursItems(favoursItems => favoursItems.filter(el=> el.id !== id))      
    }
